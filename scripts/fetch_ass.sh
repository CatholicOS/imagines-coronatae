...
set -euo pipefail
OUT="${1:-ass_pdf}"; mkdir -p "$OUT"
INDEX="https://www.vatican.va/archive/ass/index_it.htm"

curl -sL "$INDEX" -o ass_index.html

python3 - <<'PY'
import re,urllib.parse
h=open('ass_index.html',encoding='iso-8859-1').read()
docs=sorted(set(re.findall(r'href="(documents/[^"]+)"',h,re.I)))
with open('ass_urls.tsv','w') as f:
    for d in docs:
        url='https://www.vatican.va/archive/ass/'+d
...
cat ass_urls.tsv | xargs -P 8 -n 1 -d '\n' bash -c '
...
echo "--- verifying ---"
cat ass_urls.tsv | xargs -P 10 -n 1 -d '\n' bash -c '
u=$(echo "$0"|cut -f1); n=$(echo "$0"|cut -f2)
r=$(curl -sIL --max-time 60 "$u" | grep -i "^content-length" | tail -1 | tr -d "\r" | awk "{print \$2}")
l=$(stat -c%s "'"$OUT"'/$n" 2>/dev/null || echo 0)
